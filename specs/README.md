...
| [0020 — Outage tolerance](2026-09-08-0020-outage-tolerance.md) | Accepted | The refresh cron reads four services it doesn't own, and each of their outages cost a red run + a notification every 30 min for hours (six in two hours on 2026-09-08 while cerema.fr's TLS handshake timed out). The pipeline now **classifies** its failures — a new `Outage` raised only where the other side is provably at fault, `exit 75` (`EX_TEMPFAIL`) when every problem in a run was one — and CI holds the alarm until the outage passes `OUTAGE_GRACE_HOURS` (default 6, a repo variable), timed off a marker step because a held run is green by design. A format change, a 4xx or anything else still fails on the spot. Bonus: a CANDHIS outage now **degrades** a buoy instead of abandoning it, so tides + wind keep refreshing through it |
