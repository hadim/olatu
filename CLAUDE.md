...
- **Sentinel `999.999`** (CANDHIS "no data") is nulled at ingest (threshold ≥ 999.99). Don't
  reintroduce it; don't blanket-clip directions (real 0–360°). Realtime **`TempMer` = 0.0** is a
  second "no data" (thousands of rows in the 2021–25 API backfill), nulled in
  `build.read_realtime` — the exact 0.0 only; the raw reel keeps it, like it keeps 999.999.
